...
float3 stars(float3 p, float resolution)
{
    p.xz = pR(p.xz, khPI/2.);
    float3 c = float3(0.);
    float res = resolution*1.3;
...
float4 cloudTexture( float2 uv, float3 cloudColor ) {
  uv += uv+7.;
  uv *= 0.0005;
  // float f = fbm3(uvw*0.002,6)*0.5+0.5;
  float f = fbm(uv,6)*0.5+0.5;
  return mix(float4(0,0,0,1),float4(cloudColor,1),f*f);
}

float3 skyDome(float3 rd, float2 cloudOffset, float daylight, float cloudlevel, float starLevel, float resolution) {
  float bgDistance = abs(planeIntersect(float3(0),rd,float4(0,-1,0,1000)));
  float3 p = bgDistance*rd;
  float4 cloudTex = cloudTexture(p.xz+cloudOffset,cloudColor);
  ...
  // sky = mix(cloudTex,mix(cloudColor,cloudTex,0.2)*0.9,hor);
  ...
  sky = mix(sky, strs, luminance(strs)*starLevel);
